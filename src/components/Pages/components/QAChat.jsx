...
function QAChat() {
  ...
  const postReply = (idx, reply) => {
    setQuestions(
      questions.map((q, i) =>
        i === idx ? { ...q, replies: [...q.replies, reply] } : q
      )
    );
  };
  ...
  return (
    <section
      ...
      role="region"
      aria-label="Questions and chat"
    >
      <div>
        <h2 style={{ marginBottom: "12px", color: "#d5dce2" }}>
          Ask a Question
        </h2>
        <form className="question-form" onSubmit={postQuestion}>
          ...
          <button
            ...
            className="btn"
            style={{
              background: "rgba(153, 205, 50, 0.275)",
              color: "white",
              marginTop: "10px",
            }}
          >
          ...
          {questions.map((q, idx) => (
            <div key={idx} className="question">
              ...
              <form
                ...
                onSubmit={(e) => {
                  e.preventDefault();
                  const val = e.target.reply.value.trim();
                  if (val) {
                    postReply(idx, val);
